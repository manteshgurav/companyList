...
const TaxInvoiceTable = () => {
  ...
  const exportToPDF = (invoice) => {
    ...
    // Add "QUOTATION DETAILS" heading with padding
    doc.setFontSize(14);
    doc.setFont("helvetica", "bold");
    doc.text("QUOTATION DETAILS", 105, 45, { align: "center" });

    // Add invoice details
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Company: ${invoice.companyName}`, 10, 55);
    doc.text(`Date: ${format(new Date(invoice.date), "dd-MM-yyyy")}`, 10, 60);
    ...
    doc.save(`${invoice.companyName}_Quotation.pdf`);
  };
